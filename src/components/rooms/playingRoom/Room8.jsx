...
const Room8 = ({ loginRoom, waitingRoom, userCards, otherUserCards, pileCards, turn, socket }) => {
        ...
        <div className={styles.playing_room_wrapper_2} style={{ height: innerWidth > 425 ? '92%' : '85%' }}>
            <div
                ...
                <div className='d-flex justify-content-center w-100'>
                    {[newUsers[3], newUsers[4], newUsers[5]].map((user, index) => (
                        <div
                            key={user}
                            className='d-flex align-items-center flex-column'
                            style={{
                                marginRight: otherUserCards.find(obj => obj.username === user)?.imgs.length <= 7 && (index === 0 || index === 1) ? 15 : 0,
                ...
                {/* Left & right */}
                <div style={{ position: 'absolute', top: innerWidth <= 768 ? '15%' : '24%', width: '100%', height: innerWidth <= 768 ? '70%' : '50%' }}>
                    <div className='w-100 d-flex justify-content-evenly align-items-center h-100'>
                        ...
                        <div style={{ height: '100%', width: innerWidth > 572 ? '40%' : innerWidth <= 572 && innerWidth >= 530 ? '45%' : '50%', display: 'flex', flexDirection: 'column' }}>
                            {[newUsers[6], newUsers[7]].map((user, index) => (
                                <>
                                    <div
                                        className='d-flex align-items-center justify-content-center w-100 h-100'
                                    // style={{
